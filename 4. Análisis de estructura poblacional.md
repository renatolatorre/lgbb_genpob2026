...
cp /data/courses/course2026/course_files/scripts/*.sh .
```
...
```
Luego, filtraremos las variantes dentro de dichas regiones.
```bash
gzip -d HUSM_NORTH.beagle.gz
head -n1 HUSM_NORTH.beagle > HUSM_NORTH.filt.beagle
awk 'NR==FNR{SNPs[$1]=$1; next} $1 in SNPs' regions_to_exclude.bed HUSM_NORTH.beagle >> HUSM_NORTH.filt.beagle
gzip HUSM_NORTH.filt.beagle
```
> [!IMPORTANT]
...
```bash
awk 'NR==FNR{SNPs[$1]=$1; next} $2 in SNPs' regions_to_exclude.bed HUSM_NORTH.tped > HUSM_NORTH.filt.tped
cp HUSM_NORTH.tfam HUSM_NORTH.filt.tfam #No requiere filtrado ya que depende del número de individuos.
```
...
De manera similar a las variantes dentro de regiones obtenidas con CallableLoci, excluiremos variantes en alto desequilibrio de ligamiento:
```bash
gzip -d HUSM_NORTH.filt.beagle.gz
head -n1 HUSM_NORTH.filt.beagle > HUSM_NORTH.ldpruned.filt.beagle
awk 'NR==FNR{SNPs[$1]=$1; next} $1 in SNPs' LD_filter/HUSM_NORTH.filt_"$window"_"$step"_"$threshold".prune.in HUSM_NORTH.filt.beagle >> HUSM_NORTH.ldpruned.filt.beagle
gzip HUSM_NORTH.ldpruned.filt.beagle
```
> [!IMPORTANT]
...
script 5-PCAngsd.sh
```
Además, añadiremos los nombres de las muestras a cada resultado para facilitar la visualización:
```bash
basename -a $(grep "\\.bam" bam_file.list) | cut -d'.' -f1 > sample_names.txt
paste sample_names.txt HUSM_NORTH.ldpruned.filt.$maf.cov > HUSM_NORTH.ldpruned.filt.$maf.names.cov
```
...
> :exclamation: Probar incluyendo/excluyendo variantes en LD en los análisis posteirores. En el PCA, ¿cómo cambian los ejes principales y la agrupación de muestras?
