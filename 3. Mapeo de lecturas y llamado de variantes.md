...
| samtools sort -o FIELD_LAM34.bam
...
java -jar picard.jar MarkDuplicates I=FIELD_LAM34.bam O=FIELD_LAM34.dedup.bam M=FIELD_LAM34.dedup.metrics
```
> [!IMPORTANT]
...
samtools depth /home/courses/course2026/USERNAME/3-mapeo_variantes/FIELD_LAM34.dedup.bam > %j.FIELD_LAM34_depths.txt
samtools depth /home/courses/course2026/USERNAME/3-mapeo_variantes/FIELD_LAM34.dedup.bam | awk '{sum += $3} END {print sum/NR}' > %j.FIELD_LAM34_avgdepth.txt
samtools depth /home/courses/course2026/USERNAME/3-mapeo_variantes/FIELD_LAM34.dedup.bam | awk '{$3 >= 1}' | wc -l | awk '{print $1/400000000}' > %j.FIELD_LAM34_breadthdepth.txt
samtools depth /home/courses/course2026/USERNAME/3-mapeo_variantes/FIELD_LAM34.dedup.bam | awk '{$3 >= 10}' | wc -l | awk '{print $1/400000000 * 100}' > %j.FIELD_LAM34_breadthdepthmin10.txt
```
Obtenemos la profundidad promedio y el porcentaje de la secuencia de referencia que es cubierto por al menos una lectura:
```bash
cat FIELD_LAM34_avgdepth.txt
cat FIELD_LAM34_breadthdepth.txt
```
> [!IMPORTANT]
...
> [!IMPORTANT]
> :exclamation: Repetir el mapeo y obtener las mismas estadísticas para la muestra HUSM_LAM25. ¿Qué diferencias encuentras?
...
-doMaf 3 -minMapQ 30 -minQ 20 -minMaf 0.01 -doVcf 1 \
-geno_minDepth 2 -setMinDepthInd 2 -minInd 48 -remove_bads 1 \
-uniqueOnly 1 -postCutoff 0.95 -baq 1 -C 50 -ref ppa_v2.asm.fasta -out %j.HUSM_NORTH 
